...
function getPolynom(...args) {
  return (x) => [...args]
    .reduce((previous, current, index) => previous + current
      * (x ** (arguments.length - 1 - index)), 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const logStart = `${func.name}(${JSON.stringify(args).slice(1, -1)}) starts`;
    logFunc(logStart);
    const result = func(...args);
    const logEnd = `${func.name}(${JSON.stringify(args).slice(1, -1)}) ends`;
    logFunc(logEnd);
    return result;
  };
}
